...
// Local calendar date of a timestamp. `completed_at` and friends come back
// from Postgres as UTC ISO strings; slicing the first 10 characters (as
// niceDate below does for plain date columns) gives the UTC calendar day,
// not the phone's — the exact mismatch the note above warns about. In a
// UTC+ zone like Australia that makes a task completed in the early morning
// fail to match today() and vanish out of "Completed today" entirely,
// rather than just showing the wrong day label. Use this instead of
// `.slice(0, 10)` for anything that's a timestamp, not a bare date.
export function localDateOf(value) {
  if (!value) return '';
  const d = new Date(value);
  return Number.isNaN(d.getTime()) ? String(value).slice(0, 10) : ymd(d);
}
...
// ─── Context menu ────────────────────────────────────────────────────────
// A generic fixed-position popover for right-click menus: a scrim (closes on
// outside click) plus a panel placed at the pointer and clamped so it never
// spawns off the edge of the viewport. Only one can be open at a time.

let closeActiveContextMenu = null;

export function openContextMenu(x, y, node) {
  closeActiveContextMenu?.();

  const menu = el('div', { class: 'ctx-menu', role: 'menu' }, node);
  const scrim = el('button', { class: 'ctx-scrim', type: 'button', 'aria-label': 'Close menu', onclick: () => close() });
  document.body.append(scrim, menu);

  // Placed off-screen until measured, then clamped — a right-click near an
  // edge shouldn't spawn a menu the pointer can't reach.
  ...
  function close() {
    ...
    scrim.remove();
    menu.remove();
    if (closeActiveContextMenu === close) closeActiveContextMenu = null;
  }
